...
Key changes:
  - Smaller weight map (reduce the contaminate of background)
"""
...
BIN_FACTOR = int(LENGTH / WIDTH)
NOISE = False

IMG_PATH  = '/home/xz127/earth_project/Reconstruction/test3_1600_1200.jpg'
SAVE_PATH = f'/home/xz127/earth_project/Reconstruction/Results/{VERSION}/{VERSION}_{N_APERTURES}AP_combined_result.jpg'
...
def crop_center(img, cropx: int = WIDTH, cropy: int = WIDTH):
...
def main():
    ...
    print(f"Background color (BGR mean): {BG_COLOR}")

    # ── 7.2  Crop & stitch per angle (CPU, uses cv2) ────────────
    IMG_CROP = [
        crop_and_stitch_const_bg(IMG, phi, bg_color=BG_COLOR,
                                 L=LENGTH, W=WIDTH, canvas_size=C_SIZE)
        for phi in ANGLES
    ]

    # ── 7.3  Upload cropped images to GPU ───────────────────────
    # Each element: (H, W, 3) float32 GPU tensor
    img_crop_gpu = [
        torch.from_numpy(ic.astype(np.float32)).to(DEVICE)
        for ic in IMG_CROP
    ]

    # ── 7.4  Compute apertures, PSFs, MTFs, convolved FFTs (GPU) ─
    APERTURES_GPU = []
    PSFS_GPU      = []
    MTFS_GPU      = []
    G_IMG_FFTS_GPU = []   # complex64 (H, W, 3) — centred (fftshifted) spectrum
    SINGLE_AP_IMG  = []   # for optional visualisation (CPU numpy)

    for i, phi in enumerate(ANGLES):
        img_t    = img_crop_gpu[i]                        # (H, W, 3) float32

        aperture = get_physical_aperture_gpu(
            phi_ap_deg=phi, canvas_H=C_SIZE, canvas_W=C_SIZE,
            L=AP_L, W=AP_W, device=DEVICE)               # (H, W)

        psf  = get_psf_gpu(aperture)                      # (H, W) float32
        mtf  = get_mtf_gpu(psf)                           # (H, W) float32

        # Convolve image with PSF (all on GPU)
        single_ap = convolve_with_psf_gpu(img_t, psf)    # (H, W, 3)
        single_ap_result = single_ap
        
        if NOISE is True:
            single_ap_noise = add_poisson_noise_gpu(single_ap, peak=10000)
            single_ap_result = single_ap_noise

        # Compute centred FFT of the convolved image
        g_fft = fftshift2_gpu(
            torch.fft.fft2(single_ap_result.to(torch.complex64), dim=(0, 1)),
            axes=(0, 1)
        )                                                  # (H, W, 3) complex64
        ...
        SINGLE_AP_IMG.append(single_ap_result.cpu().numpy())

    print("Apertures / PSFs / MTFs / convolved FFTs computed on GPU.")
    ...
    # ── 7.6  Reconstruct group by group (GPU) ───────────────────
    ...
    # Save single aperture images
    single_ap_save_dir = os.path.join(group_save_dir, "SINGLE_IMG")
    os.makedirs(single_ap_save_dir, exist_ok=True)
    # Save each element of G_IMG_FFTS_GPU as a separate numpy file
    for i, f_img in enumerate(SINGLE_AP_IMG):
        f_img_bgr = cv2.cvtColor(
            (np.clip(f_img, 0, 1) * 255).astype(np.uint8),
            cv2.COLOR_RGB2BGR
        )
        f_path = os.path.join(single_ap_save_dir, f"image_{i}_phi{ANGLES[i]:.0f}.jpg")
        cv2.imwrite(f_path, f_img_bgr)
    print(f"Single ap images saved to {single_ap_save_dir}")

    # ── 7.7  Move results to CPU for display / saving ───────────
