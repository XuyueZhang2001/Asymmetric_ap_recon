...
Modified from hard_boundary_v3

Key changes:
  - Try to blur the boundary of aperture images & background
  - I think the result is not as good as v3, should choose v3.
"""
...
def soft_blend(center_img, bg_img, weight, sigma=50):
    sigma = 50.0
    ...
    w = weight.unsqueeze(0).unsqueeze(0).float()   # ensure float32 before conv2d # (1, 1, H, W)          
...
def main():
    ...
    w_stack_pad = np.zeros((N_APERTURES, C_SIZE, C_SIZE), dtype=bool)
    for i, phi in enumerate(ANGLES):
        wm = get_single_weight_map_np(phi, L=LENGTH-10, W=WIDTH-10, canvas_size=C_SIZE)
        w_stack_pad[i] = wm > 0
    ...
    for i, phi in enumerate(ANGLES):
        ...
        stitch_gpu = torch.from_numpy(stitch_result).to(DEVICE)
        conv_img_gpu = convolve_with_psf_gpu(stitch_gpu, psf_temp)
        ...
        # kernel_1d = kernel_1d / kernel_1d.sum()                 # normalise
        # Apply separable Gaussian blur to the weight map using F.conv2d
        weight_map_t = torch.from_numpy(w_stack_pad[i, :, :]).to(DEVICE).float()
        ...
        # # Weighted blend: weight=1 -> original, weight=0 -> convolved
        # soft_weight = soft_weight.unsqueeze(-1)                 # (H, W, 1) broadcast over RGB
        # blended_result = soft_weight * stitch_gpu + (1.0 - soft_weight) * conv_img_gpu
        single_ap_soft = soft_blend(center_img=stitch_gpu, bg_img=conv_img_gpu, 
                            weight=weight_map_t, sigma=50)
        IMG_CROP_ORI.append(stitch_result)
        IMG_CROP.append(single_ap_soft.cpu().numpy())
    ...
    f_img_save_dir = os.path.join(group_save_dir, "IMG_SINGLE_ORI")
    os.makedirs(f_img_save_dir, exist_ok=True)
    for i, f_img in enumerate(IMG_CROP_ORI):
        f_img_bgr = cv2.cvtColor(
            (np.clip(f_img, 0, 1) * 255).astype(np.uint8),
            cv2.COLOR_RGB2BGR
        )
    ...
    for i, filled_img in enumerate(IMG_CROP_FILL):
        ...
        blended_result = soft_blend(center_img=fill_img_gpu, bg_img=conv_img_gpu, 
                                    weight=weight_map_t, sigma=500)
        # w = weight_map_t.unsqueeze(0).unsqueeze(0).float()   # ensure float32 before conv2d # (1, 1, H, W)          
        # k_h = kernel_1d.view(1, 1, ksize, 1)
        # k_w = kernel_1d.view(1, 1, 1, ksize)
        # w = F.conv2d(w, k_h, padding=(half, 0))
        ...
        FINAL_IMG.append(blended_result.cpu().numpy())
