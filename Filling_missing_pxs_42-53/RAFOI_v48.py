...
Key changes:
  - Smaller weight map when replacing the conved fill image
  - Erode the weight map inward by C_SIZE/AP_L pixels on all four sides in empty pixel filling
"""
...
def main():
    ...
    for i, filled_img in enumerate(IMG_CROP_FILL):
        ...
        # Erode the weight map inward by shrink_px pixels on all four sides.
        # Pixels within shrink_px of any 0-region are set to 0 before blending.
        shrink_px = int(C_SIZE / AP_L)
        if shrink_px > 0:
            # Use max-pooling on the inverted map to dilate the 0-region,
            # which is equivalent to eroding the 1-region.
            w_inv = 1.0 - weight_map_t                                      # invert: 0->1, 1->0
            w_inv_4d = w_inv.unsqueeze(0).unsqueeze(0)                      # (1, 1, H, W)
            kernel_size = 2 * shrink_px + 1
            w_inv_dilated = F.max_pool2d(
                w_inv_4d,
                kernel_size=kernel_size,
                stride=1,
                padding=shrink_px
            )
            weight_map_t = (1.0 - w_inv_dilated).squeeze(0).squeeze(0)     # (H, W)
