...
Key changes:
  - Try to fill all the images with pixels from other single apertures
"""
...
def fill_missing_pixels(
    ...
    footprint (contributor_map non-empty) but are NOT observed by this
    aperture (weight_stack[id]==0).  Each such pixel is replaced by the
    value from the nearest aperture (in circular ID distance) that DOES
    observe that pixel.

    Args:
        img_list      : list of N_APERTURE RGB images, each (C_SIZE, C_SIZE, 3), float [0,1]
        weights       : binary ndarray (N_APERTURE, C_SIZE, C_SIZE)
        contributor_map: object ndarray (C_SIZE, C_SIZE), each element is a
                         list/array of aperture IDs that observe that pixel

    Returns:
        filled_crops  : list of N_APERTURE filled images, same shape as input
    """
    N_APERTURE = len(img_list)
    C_SIZE_H, C_SIZE_W = weights.shape[1], weights.shape[2]

    # Work on copies so the originals are not mutated
    filled_crops = [img.copy() for img in img_list]

    # Precompute the (y, x) coordinates of all pixels where at least one
    # aperture contributes — these are the only pixels we ever need to visit
    nonempty_ys, nonempty_xs = np.where(
        np.array([[len(contributor_map[y, x]) > 0
                   for x in range(C_SIZE_W)]
                  for y in range(C_SIZE_H)])
    )
    # Stack into (N_pixels, 2) for convenient iteration
    nonempty_coords = list(zip(nonempty_ys.tolist(), nonempty_xs.tolist()))

    for aid in range(N_APERTURE):
        weight_map = weights[aid]          # (C_SIZE_H, C_SIZE_W) binary

        for (y, x) in nonempty_coords:
            # Pixel already observed by this aperture — nothing to do
            if weight_map[y, x] == 1:
                continue

            # Candidate aperture IDs that DO observe this pixel
            candidates = contributor_map[y, x]  # list or 1-D array of IDs

            # Find the candidate whose circular distance to aid is smallest.
            # Circular distance between two IDs a, b in a ring of size N:
            #   d = min(|a - b|, N - |a - b|)
            best_id = None
            best_dist = N_APERTURE + 1          # larger than any possible distance

            for cid in candidates:
                linear_dist = abs(int(cid) - aid)
                circular_dist = min(linear_dist, N_APERTURE - linear_dist)
                if circular_dist < best_dist:
                    best_dist = circular_dist
                    best_id = int(cid)

            # Replace pixel value with the chosen neighbour aperture's pixel
            if best_id is not None:
                filled_crops[aid][y, x] = img_list[best_id][y, x]
...
def main():
    ...
    IMG_CROP_FILL = fill_missing_pixels(img_list = IMG_CROP, 
                                        weights = w_stack, 
