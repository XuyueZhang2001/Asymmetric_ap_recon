...
Key changes:
  - Modify fill_missing_pixels to make it run on GPU
"""
...
def fill_missing_pixels_gpu(
    ...
    Each such pixel is replaced by the value from the nearest aperture
    (in circular ID distance) that DOES observe that pixel.
    ...
    # contributors: (H, W, MAX_K)
    contributors = torch.from_numpy(contributors_np).to(device)          # int32
    valid_mask   = torch.from_numpy(valid_mask_np).to(device)            # bool
    # weights_t: (N, H, W) bool
    weights_t = torch.from_numpy(weights.astype(bool)).to(device)
    # img_stack: (N, H, W, 3)  — keep as float32
    img_stack = torch.from_numpy(
        np.stack(img_list, axis=0).astype(np.float32)
    ).to(device)

    # 3. Determine which (aperture, pixel) pairs need filling.            #
    any_contributor = valid_mask.any(dim=-1)          # (H, W)
    # Expand for broadcast: (N, H, W)
    need_fill = any_contributor.unsqueeze(0) & ~weights_t  # (N, H, W)

    # 4. For each (aperture, pixel) that needs filling, find the          #
    contributors_exp = contributors.unsqueeze(0).float()     # (1, H, W, K)
    aid_grid = torch.arange(N_APERTURE, device=device,
                            dtype=torch.float32
                            ).view(N_APERTURE, 1, 1, 1).expand(
                                N_APERTURE, H, W, max_k)     # (N, H, W, K)

    linear_dist = (contributors_exp - aid_grid).abs()        # (N, H, W, K)
    circ_dist   = torch.minimum(linear_dist,
                                N_APERTURE - linear_dist)    # (N, H, W, K)
    # Mask out invalid (padded) slots with a large sentinel distance
    INF = float(N_APERTURE + 1)
    valid_exp = valid_mask.unsqueeze(0).expand(N_APERTURE, H, W, max_k)
    circ_dist = torch.where(valid_exp, circ_dist, torch.full_like(circ_dist, INF))
    # argmin over the candidate dimension -> best_k: (N, H, W)
    best_k = circ_dist.argmin(dim=-1)                        # (N, H, W)
    # Gather the winning contributor ID for each (n, y, x)
    # contributors is (H, W, K); we need index [y, x, best_k[n,y,x]]
    # Expand contributors to (N, H, W, K) then gather along dim 3.
    contributors_4d = contributors.unsqueeze(0).expand(N_APERTURE, H, W, max_k)
    # best_k needs a trailing dim for gather
    best_src_id = contributors_4d.gather(
        3, best_k.unsqueeze(-1).long()
    ).squeeze(-1)                                            # (N, H, W)  int64

    # 5. Build the filled image stack.                                    #
    # Gather replacement pixels: for each (n, y, x), fetch img_stack[best_src_id[n,y,x], y, x, :]
    # img_stack shape: (N, H, W, 3)
    # Flatten spatial dims for simpler indexing, then restore.
    img_flat    = img_stack.view(N_APERTURE, H * W, 3)       # (N, HW, 3)
    src_flat    = best_src_id.view(N_APERTURE, H * W)        # (N, HW)
    need_flat   = need_fill.view(N_APERTURE, H * W)          # (N, HW)

    # For each (n, hw), src_flat[n, hw] is the source aperture index.
    # We need img_stack[src_flat[n,hw], hw, :].
    # Expand src_flat to index across the N dimension of img_flat.
    src_pixels = img_flat[
        src_flat.long(),
        torch.arange(H * W, device=device).unsqueeze(0).expand(N_APERTURE, H * W)
    ]                                                         # (N, HW, 3)
    ...
    # 6. Return as a list of CPU ndarrays matching the input dtype.       #
...
def main():
    ...
    for i, phi in enumerate(ANGLES):
        ...
        padded_gpu = torch.from_numpy(IMG_CROP_FILL[i].astype(np.float32)).to(DEVICE)
