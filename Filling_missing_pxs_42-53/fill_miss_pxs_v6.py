...
Key changes:
  - Try mirror padding, instead of filling missing pixels
  - Smaller weight window in recon
"""
...
SAVE_PATH = f'/home/xz127/earth_project/Reconstruction/Results/fill_miss_px_v{VERSION}/{REG_K}_{NOISE}_{BIN_FACTOR}/{N_APERTURES}AP_combined_result.jpg'
...
def get_physical_aperture_gpu(phi_ap_deg: float,
    ...
    """
    canvas_H = int(round(canvas_H))
    canvas_W = int(round(canvas_W))
...
def main():
    ...
    h_target = int(LENGTH*1.25) + 200
    w_target = int(WIDTH*1.25) + 200
    ...
    for i, phi in enumerate(ANGLES):
        ...
        roi_tensor = torch.from_numpy(roi.copy()).to(DEVICE)
        
        
        
        # Apply mirror padding
        # --- 2. Apply reflection padding ---
        img_4d = roi_tensor.permute(2, 0, 1).unsqueeze(0)
        orig_H, orig_W = roi_tensor.shape[:2]
        pad_h, pad_w = orig_H // 8, orig_W // 8
        padded_img = F.pad(img_4d, (pad_w, pad_w, pad_h, pad_h), mode='reflect').squeeze(0).permute(1, 2, 0)
        padded_img_cpu = padded_img.cpu().numpy()
        
        
        roi_padded = pad_to_size(padded_img_cpu, H_target=h_target, W_target=w_target, bg_background=BG_COLOR)
        
        roi_for_conv_gpu = torch.from_numpy(roi_padded.copy()).to(DEVICE)
        ...
        stitch_result = rotate_and_blend(img=bin_img, phi_deg=phi, canvas_H=C_SIZE, canvas_W=C_SIZE,
                                      bg_color=BG_COLOR, sigma=10)
        IMG_CROP.append(stitch_result)
        IMG_ROI.append(bin_img)
        padded_gpu = torch.from_numpy(stitch_result.astype(np.float32)).to(DEVICE)
