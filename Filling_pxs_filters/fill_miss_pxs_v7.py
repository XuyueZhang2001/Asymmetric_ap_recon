...
Modified from v5
Still original h filter

Key changes:
  - Smaller weight window in filling the missing pixels
  - In the filling process, firstly conv the background with the same PSF to the target image, 
  to make the boundary more smooth
  - Also increase the blending sigma from 50 to 500.
  - Also fixed the rotate_and_blend, to make sure the bg_color keeps the same when sigma > 0 (no empty pixels)
"""
