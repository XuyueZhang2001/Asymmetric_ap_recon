...
Modified from hard_boundary_v6
...
SAVE_PATH = f'/home/xz127/earth_project/Reconstruction/Results/hard_boundary_v{VERSION}/{REG_K}_{NOISE}_{BIN_FACTOR}/{N_APERTURES}AP_combined_result.jpg'
...
def fill_corr_px_gpu(group_id: int,
    ...
    # --- New in calibration: blur the binary mask so adjacent groups overlap ---
...
def main():
    ...
    for i, phi in enumerate(ANGLES):
        ...
        # conv_result_gpu = convolve_with_psf_gpu(roi_for_conv_gpu, psf_for_conv)
    ...
    # ── 7.5  Build weight maps and fill empty pixels ─────
    ...
    for i in range(1, num_groups + 1):
        ...
        if final_fft is not None:
            np.save(os.path.join(group_save_dir, f"{i}_final_fft.npy"), final_fft.cpu().numpy())
