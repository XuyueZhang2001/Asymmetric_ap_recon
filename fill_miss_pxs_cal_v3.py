...
Modified from fill_miss_pxs_cal_v2

Key changes:
  - Modify fill_corr_px_gpu, build a soft weight per group and accumulate
  - What changed: soft weighting, h filter remains 1 in dc
"""
...
SAVE_PATH = f'/home/xz127/earth_project/Reconstruction/Results/fill_cal_v{VERSION}/{REG_K}_{NOISE}_{BIN_FACTOR}/{N_APERTURES}AP_combined_result.jpg'
...
def combine_unequal_images_gpu(
    ...
    Args:
        k: base regularization (noise-to-signal) constant for the Wiener-like
           ...
           (sum_m_mtf) is small, preventing noise blow-up. Larger k -> more
           regularization -> smoother/less-sharp result; smaller k -> more
           aggressive deconvolution -> sharper but noisier. Here k is not
           constant across frequency: it is scaled radially by
           k_radial = k * (1 + beta * (r/r_max)**2), so regularization is
           stronger at high spatial frequencies (where noise dominates) and
           weaker near DC (where signal dominates).
    ...
    # h_filter = mtf_ideal * H_eff / (H_eff**2 + k)   # nsr = noise/signal power ratio
    beta = 10
