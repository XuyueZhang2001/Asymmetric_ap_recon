...
Modified from fill_miss_pxs_cal_v1

Key changes:
  - Modify combine_unequal_images_gpu, Force the H-filter to pass DC through unscaled
  - Normalize each group's spatial mean to the shared DC
"""
...
def combine_unequal_images_gpu(
    ...
    # ------NEW in cal: keep the calibrated DC unscaled so all groups share the same mean------
    ...
    # ------NEW in v2: rescale each group so its mean matches the shared DC mean------
