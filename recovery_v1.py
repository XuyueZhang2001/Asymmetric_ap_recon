...
Modified from fill_miss_pxs_cal_v3

Key changes:
  - Add angular term for h filter, applying rotational symmetry
"""
...
SAVE_PATH = f'/home/xz127/earth_project/Reconstruction/Results/freq_reco_v{VERSION}/{REG_K}_{NOISE}_{BIN_FACTOR}/{N_APERTURES}AP_combined_result.jpg'
...
# Crop and rotate a rectangular ROI from the input image, then paste it onto a canvas with a constant background color. 
# Optionally bin the ROI along the LENGTH direction before pasting it back, then resize back to the original ROI size via interpolation.
...
# Crop a rotated rectangular ROI with input degree from the input image, return the cropped area as rectangular image.
def crop_and_rotate(image, phi_ap_degrees, L=LENGTH, W=WIDTH):
    """
...
# Pad an image to a target size with a specified background color.
def pad_to_size(img, H_target=LENGTH+200, W_target=WIDTH+200, bg_background=(0,0,0)):
    H, W = img.shape[:2]
...
# Binning the image along longer dimension (usually along the short axis of the aperture)
def bin_blurring_direction(img, bin_factor=BIN_FACTOR):
        # Binning
...
def get_contributor_map(weights: np.ndarray, #(N_APERTURE, C_SIZE, C_SIZE), the same as weight_stack
    ...
    '''
    Produce a map to present which aperture contribute to a specific pixel
    '''
...
def add_poisson_noise(img, peak=1000.0, seed=None):
    ...
    peak: the photon count corresponding to an intensity of 1.0, effectively controlling the signal-to-noise ratio, 
        higher peak values correspond to higher signal-to-noise ratio.
    """
...
def convolve_with_psf_gpu(img_t: torch.Tensor, psf_t: torch.Tensor) -> torch.Tensor:
    """
    Convolve img_t with psf on GPU
    """
...
def combine_unequal_images_gpu(
    ...
    beta: float = 10.0,      # radial growth of regularization (high-freq noise suppression)
    gamma: float = 20.0,     # angular (wedge) growth of regularization
    mtf_ref: float = None,   # reference effective-MTF level; if None, estimated from data
    device: torch.device = DEVICE
):
    """
    ...
    Angle-aware regularization
    --------------------------
    Under the rotational-symmetry assumption, the fused effective MTF
    (sum_m_mtf) is strong along the captured spokes and weak in the wedge
    regions between adjacent capture angles. A purely radial regularization
    k_radial(r) treats all azimuths equally, so it either over-smooths the
    spokes (if large enough to tame the wedges) or amplifies noise in the
    wedges (if small enough to keep the spokes sharp). To resolve this, the
    regularization here is made a function of BOTH radius and the local
    effective MTF:

        k(r, theta) = k * (1 + beta * (r/r_max)^2) * (1 + gamma * wedge_weight)

    where wedge_weight in [0, 1] is large where H_eff is weak (wedges) and
    ~0 where H_eff is strong (spokes). This lets the inverse filter safely
    extract the low-but-real signal in the wedges (strong reg -> no noise
    blow-up) while keeping aggressive deconvolution on the spokes (weak reg
    -> sharp). It creates NO new information; it only balances noise vs.
    signal per azimuth using the measured H_eff as its own guide.

    Args:
        k: base regularization (noise-to-signal) constant for the Wiener-like
           H-filter. It bounds the inverse-filter gain where the effective MTF
           (sum_m_mtf) is small, preventing noise blow-up.
        beta: controls how much regularization grows with spatial frequency.
        gamma: controls how much extra regularization is applied in the
           angular wedges (low-H_eff regions between spokes). gamma=0 recovers
           the purely radial behavior.
        mtf_ref: reference level used to normalize the effective MTF when
           building the wedge weight. If None, it is estimated as a high
           percentile of the nonzero H_eff (i.e. the typical spoke strength).
    ...
    # Σ M_i · MTF_i  (2-D accumulator) — the fused effective MTF, H_eff
    ...
    # ---- Radial frequency term: stronger reg at high spatial frequencies ----
    fx = torch.fft.fftshift(torch.fft.fftfreq(canvas_size, device=device))
    FX, FY = torch.meshgrid(fx, fx, indexing='ij')
    r = torch.sqrt(FX**2 + FY**2)                       # normalized radius of freq domain
    radial_term = 1.0 + beta * (r / r.max())**2         # k grows toward high freq

    # ---- Angular (wedge) term: stronger reg where H_eff is weak (between spokes) ----
    # Estimate the typical "spoke" strength of the effective MTF. Using a high
    # percentile of the in-band, nonzero H_eff makes this robust to the wedges
    # themselves and to the DC spike.
    in_band = (mtf_ideal > 0) & (H_eff > 0)
    if mtf_ref is None:
        if in_band.any():
            mtf_ref = torch.quantile(H_eff[in_band], 0.90)
        else:
            mtf_ref = H_eff.max()
    mtf_ref = torch.as_tensor(mtf_ref, dtype=H_eff.dtype, device=device)
    mtf_ref = torch.clamp(mtf_ref, min=1e-8)

    # Normalized effective MTF in [0, 1]: ~1 on spokes, ~0 in wedges.
    h_norm = torch.clamp(H_eff / mtf_ref, 0.0, 1.0)
    # wedge_weight is the complement: ~0 on spokes, ~1 in the wedges.
    wedge_weight = 1.0 - h_norm
    # Only apply extra angular regularization inside the ideal passband;
    # outside the diffraction limit there is no signal to protect anyway.
    wedge_weight = wedge_weight * (mtf_ideal > 0).to(wedge_weight.dtype)
    angular_term = 1.0 + gamma * wedge_weight

    # ---- Combined angle-aware, radius-aware regularization ----
    k_map = k * radial_term * angular_term              # k(r, theta)

    # H-filter (Wiener-like) with spatially varying regularization
    h_filter = mtf_ideal / (sum_m_mtf + k_map)
    ...
    # ------rescale each group so its mean matches the shared DC mean------
    if dc_value is not None:
        ...
        # or additive: rec_spatial_h + (target_mean - cur_mean)

    reconstructed_np = rec_spatial_h.cpu().numpy().astype(np.float32)

    return rec_img_ori_np, reconstructed_np, reconstructed_fft, final_fft, masks, sum_m_mtf, h_filter, k_map
...
def fill_corr_px_gpu(group_id: int,
    ...
    Returns:
        ...
        k_map               (H, W)    float32 GPU tensor — k-map for this group
    """
    ...
    rec_no_h_np, rec_np, reconstructed_fft, final_fft, masks, sum_m_mtf, h_filter, k_map = combine_unequal_images_gpu(
        ang_id=ang_ids, k=k,
    ...
    # Build the group's hard mask, then feather it
    mask_np  = (GROUP_ID_ARR == group_id)                          # (H, W) bool
    mask_gpu = torch.from_numpy(mask_np).to(device).float()        # (H, W)

    # --- feather: blur the binary mask so adjacent groups overlap ---
    sigma = 15.0                                                   # 10–30 is reasonable
    ...
    return (acc_num, acc_den, acc_num_no_h, acc_den_no_h,
            rec_no_h_np, rec_np, reconstructed_fft, final_fft, masks, sum_m_mtf, h_filter, k_map)
...
def main():
    ...
    h_target = LENGTH + 200
    w_target = WIDTH + 200
    aperture_for_conv = get_physical_aperture_gpu(phi_ap_deg=0, 
                                                  canvas_H=h_target, canvas_W=w_target, L=AP_L, W=AP_W, 
                                                  device=DEVICE)
    psf_for_conv = get_psf_gpu(aperture_for_conv)
    
    IMG_ROI = [] # Store the cropped regions of interest
    IMG_CROP = [] # Store the stitched crop results (stitch it to the larger pure-color canvas)
    for i, phi in enumerate(ANGLES):
        roi = crop_and_rotate(IMG_FULL, phi_ap_degrees=phi, L=LENGTH, W=WIDTH)
        roi_padded = pad_to_size(roi, H_target=h_target, W_target=w_target, bg_background=BG_COLOR)
        
        
        IMG_ROI.append(roi)
        roi_for_conv_gpu = torch.from_numpy(roi_padded).to(DEVICE)
        conv_result_gpu = convolve_with_psf_gpu(roi_for_conv_gpu, psf_for_conv) # Convolve the padded ROI with the PSF on GPU
        conv_clean = conv_result_gpu.cpu().numpy()
        conv_result = conv_clean
        if NOISE is True:
            conv_noise = add_poisson_noise(conv_clean)
            conv_result = conv_noise
        bin_img = bin_blurring_direction(conv_result, BIN_FACTOR)
    ...
    w_stack = np.zeros((N_APERTURES, C_SIZE, C_SIZE), dtype=bool)
    for i, phi in enumerate(ANGLES):
        wm = get_single_weight_map_np(phi, L=LENGTH - 30, W=WIDTH - 30, canvas_size=C_SIZE)
        w_stack[i] = wm > 0
    ...
    # ── Recompute apertures/PSFs/MTFs at canvas size if needed ──
    if ker_size != C_SIZE:
    ...
    # --------------------blur the pxs out side the weighting map, to make the transition smoother between filled and original pixels
    ...
    weight_stack, TOTAL_WEIGHT_MAP, GROUP_ID_ARR, GROUP_MAP, rows, cols = build_weight_maps(
        angles=ANGLES, canvas_size=C_SIZE, L=LENGTH - 50, W=WIDTH - 50
    )
    ...
    # -------Compute mean DC value from all single-FFT images for DC replacement in reconstruction-------
    # By normalizing the DC component of all the single reconstructed images, 
    # so all the reconstructed images have a consistent DC level (brightness)
    ...
    for i in range(1, num_groups + 1):
        ...
        (acc_num, acc_den, acc_num_no_h, acc_den_no_h, rec_no_h_np, 
         rec_np, reconstructed_fft, final_fft, masks, sum_m_mtf, h_filter, k_map) = fill_corr_px_gpu(
            group_id=i,
        ...
        if k_map is not None:
            np.save(os.path.join(group_save_dir, f"{i}_k_map.npy"), k_map.cpu().numpy())
